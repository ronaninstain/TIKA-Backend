                    ...
                    <table class="w-full border-r border-b">
                        ...
                        <tr>
                            <td class="border-l border-t px-2 py-1 text-left">
                                @if($person->registered == 0) <del> @endif
                                {{$person->dob}}
                                @if($person->registered == 0) <del> @endif
                            </td>
                            <td class="border-l border-t px-2 py-1 text-center">
                                <a class="inline-block" href="{{route('people.edit', $person->id)}}">Edit</a>
                                <form action="{{route('people-edit', $person->id)}}" method="POST" class="inline-block"> @csrf
                                    <button type="submit">{{$person->registered == 0 ? 'Restore' : 'Archive'}}</button>
                                </form>
                            </td>
                        </tr>
                        @endforeach
